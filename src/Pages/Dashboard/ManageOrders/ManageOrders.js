...
const ManageOrders = () => {
    ...
    useEffect(() => {
        const url = `https://pacific-sands-70895.herokuapp.com/orders`
        fetch(url, {
            headers: {
        ...
        fetch('https://pacific-sands-70895.herokuapp.com/orders', {
            ...
            .then(data => {
                if (data.modifiedCount > 0) {
                    const restOrders = allOrders.filter(item => item._id !== updatedOrder._id);
        ...
        <div>
            ...
            <div className="overflow-x-auto">
                <table className="table w-full">
                    ...
                    <tbody>
                        {
                            allOrders.map((item, index) => <tr key={item._id}>
                                <td>
                                    {index + 1}
                                </td>
                                <td>
                                ...
                                <th>
                                    <div className="flex flex-auto flex-col gap-3">
                                        <button
                                            ...
                                            className={item.status ? 'bg-green-600 opacity-25 w-full hover:bg-green-700 px-4 py-1 rounded text-white ' : 'bg-green-600 hover:bg-green-700 w-full px-4 py-1 rounded text-white'}
                                            disabled={item.status}
                                        >Approve</button>
                                        <button onClick={() => handleCancellation(item._id)} className="bg-red-700 w-full hover:bg-red-800 px-4 py-1 rounded text-white m-auto">Cancel</button>
